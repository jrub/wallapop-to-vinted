...
2. **Refactor: Page Object + domain modules — TDD-driven, in 5 ordered phases.** `upload_vinted.py` is over 1500 lines and mixes DOM selectors with domain logic. Without tests this isn't a refactor, it's a rewrite — the upload flow is too tangled to move safely in one shot. Plan: write tests first for each layer, then move code, layer by layer.
   ...
   **Step 4 — remaining Page Objects. 🚧 IN PROGRESS (2026-05-01).** `LoginPage` extracted to `vinted/pages/login.py`; raises `LoginFailed` (was bare `RuntimeError`) and uses `vinted.session.abort_if_captcha` directly. Stealth tests moved to `tests/vinted/pages/test_login.py`. Navigation AST guard now parametrised over `upload_vinted.py` + `vinted/pages/login.py` so any new `page.goto` without `wait_until=` fails on either file. `upload_vinted.py` down from 1042 → ~984 lines. 189 tests, 1 skipped. **Pending:** capture `new_item_books` fixture (needs `--visible`); extract `EditDraftPage` (`vinted/pages/edit_draft.py`) and `ProfilePage` (`vinted/pages/profile.py`).
